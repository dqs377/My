...
cc.Class({
    ...
    onLoad: function () {
        ...
        clickEventHandler.component = "zuo";// 这个是代码文件名
    ...
    callback: function (event, customEventData) {
        ...
        if(this.player.x <= 250 && this.player.x > 0){
            if(!(this.player.x==200 && this.player.y == 50))
            if(!(this.player.x==200 && this.player.y == 100))
            if(!(this.player.x==150 && this.player.y == 200))//不能够往左移动的格子  玩家左移，脚本赋予“left”按钮
            if(!(this.player.x==100 && this.player.y == 0))
            if(!(this.player.x==100 && this.player.y == 200))
            if(!(this.player.x==250 && this.player.y == 50))
            {
                this.player.x -= 50;//向左移动一格
